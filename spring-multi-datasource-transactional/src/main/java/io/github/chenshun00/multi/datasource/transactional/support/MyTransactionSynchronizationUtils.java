 ...
 * @since 2021/6/7 9:55 上午
 */
public class MyTransactionSynchronizationUtils {

    private static final Log logger = LogFactory.getLog(MyTransactionSynchronizationUtils.class);

    private static final boolean aopAvailable = ClassUtils.isPresent(
            "org.springframework.aop.scope.ScopedObject", MyTransactionSynchronizationUtils.class.getClassLoader());
     ...
     */
    public static void triggerFlush(String datasource) {
        for (TransactionSynchronization synchronization : MyTransactionSynchronizationManager.getSynchronizations(datasource)) {
     ...
     */
    public static void triggerBeforeCommit(String datasource, boolean readOnly) {
        for (TransactionSynchronization synchronization : MyTransactionSynchronizationManager.getSynchronizations(datasource)) {
     ...
     */
    public static void triggerBeforeCompletion(String datasource) {
        for (TransactionSynchronization synchronization : MyTransactionSynchronizationManager.getSynchronizations(datasource)) {
            try {
     ...
     */
    public static void triggerAfterCommit(String datasource) {
        invokeAfterCommit(MyTransactionSynchronizationManager.getSynchronizations(datasource));
    }
     ...
     */
    public static void triggerAfterCompletion(String datasource, int completionStatus) {
        List<TransactionSynchronization> synchronizations = MyTransactionSynchronizationManager.getSynchronizations(datasource);
